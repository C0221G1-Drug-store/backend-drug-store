...
//    List<ImportBill>getAllImportBill(int index);
    ...
    @Query(value = "select * from import_bill where (import_system_code like :importCode) and (invoice_date between :startDate  and :endDate)", nativeQuery = true)
    List<ImportBill> searchBill(@Param("importCode") String importCode,@Param("startDate") String startDate,@Param("endDate") String endDate);
}
