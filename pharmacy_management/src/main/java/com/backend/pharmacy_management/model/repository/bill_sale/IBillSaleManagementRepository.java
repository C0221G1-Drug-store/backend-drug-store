...
public interface IBillSaleManagementRepository extends PagingAndSortingRepository<BillSale,Long> {
    ...
    @Query(value = "select * from bill_sale where date(invoice_date) between cast(?1 as date) and cast(?2 as date) and time(invoice_date) between cast(?3 as time ) and cast(?4 as time) and bill_sale_type like %?5% and flag = 0", nativeQuery = true )
    List<BillSale> findBillBySearch(String search1 , String search2, String search3, String search4 , String search5);
    @Query(value = "select * from bill_sale where date(invoice_date) between cast(?1 as date) and cast(?2 as date) and time(invoice_date) between cast(?3 as time ) and cast(?4 as time) and bill_sale_type like %?5% and flag = 0 order by bill_sale_code desc ", nativeQuery = true )
    List<BillSale> findBillBySearch1(String search1 , String search2, String search3, String search4 , String search5);

    @Query(value = "select * from bill_sale left join customer on bill_sale.customer_id = customer.customer_id where date(invoice_date) between cast(?1 as date) and cast(?2 as date) and time(invoice_date) between cast(?3 as time ) and cast(?4 as time) and bill_sale_type like %?5% and flag = 0 order by customer_name asc ", nativeQuery = true )
    List<BillSale> findBillBySearch2(String search1 , String search2, String search3, String search4 , String search5);
    @Query(value = "select * from bill_sale where date(invoice_date) between cast(?1 as date) and cast(?2 as date) and time(invoice_date) between cast(?3 as time ) and cast(?4 as time) and bill_sale_type like %?5% and flag = 0 order by invoice_date asc ", nativeQuery = true )
    List<BillSale> findBillBySearch3(String search1 , String search2, String search3, String search4 , String search5);
    @Query(value = "select * from bill_sale left join employee on bill_sale.employee_id = employee.employee_id where date(invoice_date) between cast(?1 as date) and cast(?2 as date) and time(invoice_date) between cast(?3 as time ) and cast(?4 as time) and bill_sale_type like %?5% and flag = 0 order by employee_name asc ", nativeQuery = true )
    List<BillSale> findBillBySearch4(String search1 , String search2, String search3, String search4 , String search5);
    @Query(value = "select * from bill_sale where date(invoice_date) between cast(?1 as date) and cast(?2 as date) and time(invoice_date) between cast(?3 as time ) and cast(?4 as time) and bill_sale_type like %?5% and flag = 0 order by total_money asc ", nativeQuery = true )
    ...
    @Query(value="select * from bill_sale where bill_sale_id = ?1 and flag = 0", nativeQuery = true)
    Optional <BillSale> findBillSaleById(long id);

    @Transactional
    @Modifying
    @Query(value = "update bill_sale set flag = 1 where bill_sale_id = ?1", nativeQuery = true)
    ...
    @Query(value = "select * from bill_sale left join customer on bill_sale.customer_id = customer.id where date(invoice_date) between cast(?1 as date) and cast(?2 as date) and time(invoice_date) between cast(?3 as time ) and cast(?4 as time) and bill_sale_type like %?5% and flag = 0 order by customer_name asc ", nativeQuery = true )
    ...
    @Query(value = "select * from bill_sale left join employee on bill_sale.employee_id = employee.id where date(invoice_date) between cast(?1 as date) and cast(?2 as date) and time(invoice_date) between cast(?3 as time ) and cast(?4 as time) and bill_sale_type like %?5% and flag = 0 order by employee_name asc ", nativeQuery = true )
